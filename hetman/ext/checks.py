"""Hetman decorators for use with discord.py app commands.

A set of decorators for use with discord.py application commands.
These are generally Hetman specific, but some may be more general.
They require the client to be a `hetman.bot.Hetman` instance.
...
def is_owner_check():
    ...
    async def is_owner(interaction: discord.Interaction) -> bool:
        ...
        Raises:
            `hetman.exceptions.ECheckFailure`: Requirements not met.
        ...
        if interaction.user == app.owner:
            return True
        raise exceptions.ECheckFailure("You do not have permission to do this.")
